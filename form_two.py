...
class FormTwo:
	@staticmethod
	def submit(username, fullname, email, appeal_reason, proxy):
		...
		with Xvfb() as xvfb:
			...
			e_fullname = driver.find_elements_by_css_selector('form input[name="name"]')
			...
			e_appeal_reason  = driver.find_element_by_css_selector('form input[name="appeal_reason"]')
